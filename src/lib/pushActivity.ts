...
import type { JobTrackerDashboardSummaryV1 } from "../dashboard/summary.schema";
...
type JobTrackerActivity = {
  event: string;
  occurredAt: string;
  entityId?: string;
};

export const pushJobTrackerActivity = async (params: {
  userId: string;
  activity: JobTrackerActivity;
  summary: JobTrackerDashboardSummaryV1;
}): Promise<void> => {
  try {
    ...
    if (import.meta.env.DEV) {
      console.warn("pushJobTrackerActivity failed", error);
    }
  }
};
